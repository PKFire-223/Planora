...
import { CourseQuickContentModal } from './CourseQuickContentModal';
...
export function TimetableView({ 
  ...
  const [timetable, setTimetable] = useState<TimetableEntry[]>(() => {
    try {
      ...
      return courses.length > 0 ? syncCoursesToTimetable(courses, base) : base;
    } catch {
  ...
  const [filterTab, setFilterTab] = useState<'all' | 'unassigned' | 'courses'>('all');
  ...
  // Quick Course Content modal state
  ...
  const filteredItems = useMemo(() => {
    return timetable.filter(item => {
      ...
      if (filterTab === 'courses') {
        return Boolean(item.courseId || item.courseCode);
      }
      return true;
    });
                ...
                return (
                  ...
                  >
                    <div>
                      <div className="flex items-start justify-between gap-1 mb-1.5">
                        ...
                        <div className="flex items-center gap-0.5 shrink-0">
                          {matchedCourse && (
                            <button
                              title="Xem nội dung bài giảng & đề cương"
                              onClick={(e) => {
                                e.stopPropagation();
                                setSelectedCourseForContent(matchedCourse);
                              }}
                              className="p-1 rounded text-indigo-600 dark:text-indigo-400 hover:bg-indigo-500/15 transition-colors cursor-pointer"
                            >
                    ...
                    <div className="mt-2 pt-2 border-t border-slate-100 dark:border-neutral-800 text-[11px] flex items-center justify-between">
                      ...
                      {matchedCourse && (
                        <span className="text-[10px] text-slate-400 dark:text-neutral-500 font-medium shrink-0">
                          {matchedCourse.completedLessons}/{matchedCourse.totalLessons} bài
                        </span>
                      )}
                    </div>
                  </div>
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        {matchedCourse && (
                          <button
                            ...
                            className="p-0.5 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-500/15 rounded cursor-pointer"
                          >
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        {matchedCourse && (
                          <button
                            ...
                            className="p-0.5 text-indigo-600 hover:bg-indigo-500/15 rounded cursor-pointer"
                          >
                    ...
                    <div className="flex items-center gap-0.5 shrink-0">
                      {matchedCourse && (
                        <button
                          ...
                          className="p-0.5 text-indigo-600 rounded cursor-pointer"
                        >
                          <BookOpen className="w-2.5 h-2.5" />
                        </button>
                      )}
  ...
  return (
    <div className="space-y-4">
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                          {matchedCourse && (
                            <button
                              title="Xem nội dung bài giảng"
                              onClick={(e) => {
                                e.stopPropagation();
                                setSelectedCourseForContent(matchedCourse);
                              }}
                              className="p-1 rounded text-indigo-600 dark:text-indigo-400 hover:bg-indigo-500/15 cursor-pointer"
                            >
                              <BookOpen className="w-3.5 h-3.5" />
                            </button>
                          )}
      ...
      {/* ========================================================================= */}
      {/* QUICK COURSE CONTENT MODAL (XEM BÀI GIẢNG & ĐỀ CƯƠNG TRỰC TIẾP TỪ TKB)      */}
      {/* ========================================================================= */}
      <CourseQuickContentModal
        course={selectedCourseForContent}
        isOpen={Boolean(selectedCourseForContent)}
